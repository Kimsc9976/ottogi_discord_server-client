...
connections.on('connection', async socket=>{ // socket => client
    ...
    socket.on('joinRoom', async({roomName}, callback) => {
        ...
        peers[socket.id] = {
            ...
            peerDetails : {
                name : '',
                isAdmin : false, // Is first producer?
            }
        }
...
//             router = await worker.createRouter(config.mediasoup.router) // if there is not router we will make router
//         }
